...
Скрива вътрешните детайли на обекта и предоставя достъп до тях само чрез определени интерфейси (методи или свойства). Така се постига контролиран достъп в обекта. Инкапсулацията помага за контролиране на достъпа до състоянието и поведението на обекта.
...
# *❗Композицията означава създаване на екземпляри, които имат препратки към други обекти.*

# **✅Cohesion and Coupling**

⚪**Cohesion**

Коегзията се отнася до степента, до която елементите в модул или компонент са свързани помежду си и работят заедно към обща цел.

Висока кохезия означава, че елементите в модул или компонент са тясно свързани и се фокусират върху изпълнение на една, добре дефинирана задача или отговорност.

⚪**Coupling**

Loose coupling се отнася до това колко компонент на кода знае за вътрешната работа на друг компонент на кода. Ако зависи от твърде много знания за определен компонент, тогава той се счита за силно свързан. Когато даден компонент не знае твърде много за вътрешната работа на други компоненти, той се счита за слабо свързан.
...
| Структура   | Предимства                              | Недостатъци                                  | Приложения                              |
|--------------|-----------------------------------------|-----------------------------------------------|-----------------------------------------|
| Array List   | Бърз достъп, ефективно добавяне/премахване в края | Бавно в средата, фиксирана памет             | Списъци, задачи, контакти, други структури |
| Linked List  | Ефективно в средата, динамична памет      | Бавен достъп, повече памет                   | Стекове, опашки, динамични данни         |
| Stack        | Прост, ефективно добавяне/премахване      | Неподходящ за произволен достъп              | Отмяна, балансиране на скоби, обратна нотация |
| Queue        | Прост, ефективно добавяне/премахване      | Неподходящ за произволен достъп              | Симулация на опашки, мрежов трафик, задачи |
